  ...
  void reconnect()
  {
    while (!client.connected())
    {
      ...
      if (client.connect(MQTT_DEVICE_NAME, mqtt_user, mqtt_pass))
      {
        ...
        // Use MQTT_DEVICE_NAME for topic strings
        String commandTopic = String(MQTT_DEVICE_NAME) + "/commands";
        String stateTopic = String(MQTT_DEVICE_NAME) + "/state";

        client.subscribe(commandTopic.c_str());
        client.publish(stateTopic.c_str(), "ready");
      }
      else
      {
